...
# التحقق من حظر المستخدم وحذف سلعه
...
# تنظيف المنتجات المنتهية أو للمستخدمين المحظورين
    ...
    try:
        ...
        for product in products:
            if product["product_id"] == product_id:
                currency = {"crypto": "عملة مشفرة", "dollar": "دولار", "stars": "نجوم تلغرام", "transfer": "تحويل دولي", "all": "جميع وسائل الدفع", "bot_points": "نقاط بوتات"}[product['currency']]
...
def show_search_results(message_or_call, products, keyword, page):
    ...
    for idx, product in enumerate(products_to_show):
        btn = types.InlineKeyboardButton(f"{product['name']}", callback_data=f"view_{products.index(product)}")
        markup.add(btn)
...
def get_price(message):
    ...
    if message.text.isdigit():
        ...
        btn_bot_points = types.InlineKeyboardButton("نقاط بوتات", callback_data="bot_points")  # عملة جديدة
...
def get_extra_info(message):
    user_id = message.from_user.id
    extra_info = message.text if message.text.lower() != "لا شيء" else "لا توجد معلومات إضافية"
    ...
    callback_data = f"approve_{user_id}_{product_id}_{product_name}_{price}_{currency}"
    if len(callback_data) > 64:  # Telegram يحدد طول callback_data بـ 64 بايت
        product_name = product_name[:20]  # قص الاسم إذا كان طويلاً
        callback_data = f"approve_{user_id}_{product_id}_{product_name}_{price}_{currency}"
    
    markup = types.InlineKeyboardMarkup()
    btn_approve = types.InlineKeyboardButton("موافقة ✅", callback_data=callback_data)
    btn_reject = types.InlineKeyboardButton("رفض ❌", callback_data=f"reject_{user_id}")
    markup.add(btn_approve, btn_reject)
    
    currency_display = {"crypto": "عملة مشفرة", "dollar": "دولار", "stars": "نجوم تلغرام", "transfer": "تحويل دولي", "bot_points": "نقاط بوتات", "all": "جميع وسائل الدفع"}[currency]
...
def handle_approve(call):
    try:
        ...
        # البحث عن المعلومات الإضافية من user_data إذا كانت موجودة، أو تعيين افتراضي
        extra_info = user_data.get(user_id, {}).get("extra_info", "لا توجد معلومات إضافية")
        ...
        product = {
            ...
            "price": price,
            "currency": currency,
            "extra_info": extra_info,  # إضافة المعلومات الإضافية
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
        approved_products = clean_expired_products()
        approved_products.insert(0, product)
        save_products(approved_products)
...
def handle_reject(call):
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                          text="تم رفض المنتج ❌ (البيانات غير متاحة)")
    if not check_blocked_and_clean(user_id):
...
def show_stats(call):
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                          text=f"📊 *إحصائيات بوت متجرك*\n\n"
                               f"عدد السلع المعروضة: {len(products)}\n"
...
def show_products(call, page):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    for idx, product in enumerate(products_to_show):
        btn = types.InlineKeyboardButton(f"{product['name']}", callback_data=f"view_{approved_products.index(product)}")
        markup.add(btn)
...
def view_product(call):
    product_idx = int(call.data.split("_")[1])
    approved_products = clean_expired_products()
    if product_idx >= len(approved_products) or product_idx < 0:
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text="عذرًا، هذا المنتج لم يعد متاحًا!")
        bot.answer_callback_query(call.id)
        return
    
    product = approved_products[product_idx]
    currency = {"crypto": "عملة مشفرة", "dollar": "دولار", "stars": "نجوم تلغرام", "transfer": "تحويل دولي", "bot_points": "نقاط بوتات", "all": "جميع وسائل الدفع"}[product['currency']]
    extra_info = product.get("extra_info", "لا توجد معلومات إضافية")
    
    markup = types.InlineKeyboardMarkup()
    btn_buy = types.InlineKeyboardButton("شراء 🛒", callback_data=f"buy_{product_idx}")
